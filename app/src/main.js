...
if (isElectron) {
  document.getElementById("web-warning").textContent =
    "✓ Mode Electron — toutes les fonctionnalités disponibles";
  document.getElementById("web-warning").style.background =
    "rgba(166,227,161,0.1)";
  document.getElementById("web-warning").style.color = "var(--success)";
  document.getElementById("web-warning").style.borderColor =
...
function toast(message, type = "success") {
  if (isElectron) {
    window.electronAPI.toast(message, type);
  } else {
...
document.addEventListener("mousemove", (e) => {
  ...
  const newLeft = Math.max(
    200,
    Math.min(window.innerWidth - 200, startLeft + dx),
  );
